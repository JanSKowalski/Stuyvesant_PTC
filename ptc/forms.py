...
class AddForm(Form):
    search_field = StringField('add', validators=[DataRequired()])
    submit = SubmitField()
...
#Remove Parent to a teacher queue, validate ID is next parent
class RemoveForm(Form):
    ...
    def validate_id(self):
        parent_id = self.search_field.data
        parent = models.PTQueue.query.get(parent_id)
        if parent is None:
            tmp = list(self.search_field.errors)
            tmp.append("This ID is not recognized.")
            self = tuple(tmp)
